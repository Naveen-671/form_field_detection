...
_PATTERN_EMPTY_BRACKET  = re.compile(r"^\s*(\[|\()<*[\sxvX\*]*>*(>|\]|\))\s*$")          # "[ ]" or "[]" or "( )" or "[x]" etc
...
def render_numbered_boxes(
    image: Image.Image,
    proposals: List[Dict],
) -> Image.Image:
    """
    Draw numbered red bounding boxes on a copy of the image so the
    LLM can SEE all the proposals and refer to them by index.

    Returns a new PIL.Image.Image.
    """
    base = image.convert("RGBA")
    overlay = Image.new("RGBA", base.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)

    # Use a semi-transparent red for boxes
    ...
    for p in proposals:
        ...
        try:
            ...
            # Approximation of text size: width ~ 6 * len, height ~ 10
            tw = len(label) * 8
            th = 15
            draw.rectangle([x1, max(0, y1-th), x1+tw, y1], fill=text_bg)
            draw.text((x1 + 2, max(0, y1-th) + 1), label, fill=(255, 255, 255, 255))
        except Exception:
            pass
...
def build_indexed_proposals(
    ...
    for p in detector_proposals:
        ...
        if ocr_tokens:
            margin = 35.0
            exp = [p.bbox[0]-margin, p.bbox[1]-margin, p.bbox[2]+margin, p.bbox[3]+margin]
            for tok in ocr_tokens:
                # overlap with expanded bbox
                if _overlap_area(exp, tok.bbox) > 0:
